...
export default function Signin() {
  ...
  const [button_array_index, setbutton_array_index] = useState(0);
    ...
    try {
      ...
      if(showCameraLogin){
        if(userSnap.exists() && userData?.details_status==false){
          setShowModal("Personal Details Not Finished"); // Show the modal instead of alert
          setShowModal2("Finish Personal Detail to get access to camera login and all other features.")
          setbutton_array_index(2);
        }
          ...
          setShowModal("User Not Registered"); // Show the modal instead of alert
          setShowModal2("You need to sign up before trying to sign in. Please create an account first.")
          setbutton_array_index(0);
        }
      }
      else{
        if(userSnap.exists() && userData?.details_status==false){
          ...
          setShowModal("Personal Details Finished Already"); // Show the modal instead of alert
          setShowModal2("You can click the below button to access camera login and all features..")
          setbutton_array_index(1);
        }
        else {
          setShowModal("User Not Registered"); // Show the modal instead of alert
          setShowModal2("You need to sign up before trying to sign in. Please create an account first.")
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-950 via-teal-950 to-cyan-950 flex items-center justify-center overflow-hidden relative">
      ...
      {/* Content container */}
      <div
  className={`relative z-10 w-full max-w-6xl flex flex-col lg:flex-row rounded-3xl shadow-2xl transform transition-all duration-1000
    ${animationCompleted ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"} mt-4 mb-2`}
>
        
        {/* Left panel - Branding */}
        <div className="lg:w-2/5 bg-gradient-to-br from-teal-600/20 to-emerald-600/20 backdrop-blur-md p-12 rounded-t-3xl lg:rounded-l-3xl lg:rounded-tr-none flex flex-col justify-between border border-white/10 relative overflow-hidden">
          ...
          <div className="perspective-1000">
            <div className="relative transform transition-transform duration-700 hover:rotate-y-12 hover:scale-105 cursor-pointer">
              ...
              <div className="bg-gradient-to-tr from-teal-900/40 to-emerald-900/40 backdrop-blur-lg rounded-2xl p-8 border border-white/10 shadow-xl">
                <div className="flex items-center mb-8">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-r from-teal-500 to-emerald-600 flex items-center justify-center shadow-lg">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-teal-200 text-sm">Access your features</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center">
                    ...
                    <span className="ml-3 text-gray-300">Resume practice</span>
                  </div>
                  <div className="flex items-center">
                    <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-green-400" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <span className="ml-3 text-gray-300">Track your progress</span>
                  </div>
                  <div className="flex items-center">
                    <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-green-400" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <span className="ml-3 text-gray-300">Access saved materials</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-8 bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10 shadow-lg">
            <div className="flex items-center mb-4">
              <div className="flex -space-x-2">
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-teal-400 to-teal-600 flex items-center justify-center text-white text-xs font-bold">RK</div>
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-emerald-400 to-emerald-600 flex items-center justify-center text-white text-xs font-bold">SV</div>
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-cyan-400 to-cyan-600 flex items-center justify-center text-white text-xs font-bold">PT</div>
              </div>
              <div className="ml-4">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
            ...
            <p className="text-gray-300 text-sm italic">"This platform helped me land my dream job! The practice interviews and resources were invaluable."</p>
          </div>
          
          {/* Brand name */}
          <div className="mt-8 text-center lg:text-left">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-teal-400 to-emerald-500 bg-clip-text text-transparent">PlacementPrep
            </h1>
            <p className="text-gray-400 mt-2">Your gateway to career success</p>
          </div>
        </div>
        ...
        <div className="lg:w-3/5 bg-gray-900/60 backdrop-blur-lg p-12 rounded-b-3xl lg:rounded-r-3xl lg:rounded-bl-none border border-white/10 flex items-center justify-center">
          <div className="w-full max-w-md">
            ...
            <p className="text-teal-300 mb-8">Sign in to access your account</p>
            ...
            <div className="space-y-4">
              ...
              {showCameraLogin && (
                <button 
                  ...
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white py-3 px-4 rounded-xl flex items-center justify-center gap-3 font-medium transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden"
                >
      ...
      {/* Modal for User Not Registered */}
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 backdrop-blur-md z-50">
          <div className="bg-gray-900/80 backdrop-blur-lg border border-red-500/30 rounded-2xl p-8 shadow-2xl w-full max-w-md transform transition-all animate-modal-in">
            ...
            <h3 className="text-2xl text-white font-bold text-center mb-2">{showModal}</h3>
            <p className="text-gray-300 text-center mb-8">{showModal2}
            </p>
            <div className="flex justify-center">
              <button
                className="px-8 py-3 bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white rounded-xl shadow-lg hover:shadow-xl transition-all font-medium"
                onClick={() => {
                  setShowModal("");
                  setShowModal2("");
                  if(button_array_index==0){
                    navigate("/signup")
                  }
                  else if(button_array_index==1){
                    navigate("/signin")
                  }
                  else{
              ...
              >
                {button_array[button_array_index]}
              </button>
            </div>
          </div>
        </div>
