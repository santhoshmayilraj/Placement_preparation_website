...
const Company: React.FC = () => {
  // State to store the MongoDB data
  ...
  // State for transformed data and companies
  const [companyData, setCompanyData] = useState<CompanyDataForCardDisplay[]>([]);
  const [companies, setCompanies] = useState<CompanyDataForCardDisplay[]>([]);
  // Stats state
  const [stats, setStats] = useState({
    totalCompanies: 0,
    currentYearCompanies: 0,
    topRecruiter: {
      name: "N/A",
      logo: "",
      hired: 0
    ...
    fetch("http://localhost:5000/api/temp")
      ...
      .then(data => {
        ...
        if (Array.isArray(data)) {
          ...
          dataArray = [data]; // Wrap single object in an array
        } else {
          console.error("Unexpected data format:", data);
          dataArray = [];
        }
        setMongoData(dataArray);
        setIsLoading(false);
      })
      .catch(error => {
  ...
  // Transform data when mongoData changes
  useEffect(() => {
    if (mongoData.length > 0) {
      // Transform the data
      const transformedData = mongoData.map((company) => ({
        id: company.id,
        name: company.name,
        logo: company.logo,
      ...
      const current_year_comp = transformedData.filter(company => company.year === current_year).length;

      const maxHiredCompany = transformedData.length > 0
        ? transformedData.reduce((max, company) => (company.hired > max.hired ? company : max), transformedData[0])
        : { name: "N/A", logo: "", hired: 0 };
      ...
      setCompanies(transformedData); // Set initial companies to show all data
      setStats(newStats);
    }
  }, [mongoData]);
  ...
  }, [searchTerm, selectedYear, selectedDepartment, companyData]);
