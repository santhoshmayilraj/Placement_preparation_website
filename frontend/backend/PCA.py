...
class pca_class:
    ...
    def reduce_dim(self):
        p, d, q = s_linalg.svd(self.images, full_matrices=True) #Many eigen-vectors will be true, so we are telling expliciy our function
        # to include them also after the decomposition.
    ...
    def __init__(self, images, y, target_names, no_of_elements, quality_percent):
        ...
        # the direction along which mean will
        # be calculated along the row. so only 1.
        # for finding the mean along, the features.
        # we will find the mean for the firs-pixel for all the image.
        # similarly for second-pixel, and so on.
